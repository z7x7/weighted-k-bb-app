...
st.title("Strikeout Prop Diagnostic (17-Metric Analyzer)")
...
babip_rhb = st.number_input("BABIP vs RHB", min_value=0.0, max_value=1.0, value=0.280)
xfip_fip_rhb = st.number_input("xFIP - FIP vs RHB", value=0.2)
...
babip_lhb = st.number_input("BABIP vs LHB", min_value=0.0, max_value=1.0, value=0.260)
xfip_fip_lhb = st.number_input("xFIP - FIP vs LHB", value=0.4)
...
k_home = st.number_input("Home Avg Ks/Game", min_value=0.0, value=8.0)
k_away = st.number_input("Away Avg Ks/Game", min_value=0.0, value=8.7)
...
if st.button("Analyze"):
    ...
    results = {
        ...
        "xFIP - FIP vs RHB": evaluate_metric(xfip_fip_rhb, {"over": 0.0, "neutral_low": -0.3, "neutral_high": 0.3}) if xfip_fip_rhb <= 1.0 else "Under",
        "xFIP - FIP vs LHB": evaluate_metric(xfip_fip_lhb, {"over": 0.0, "neutral_low": -0.3, "neutral_high": 0.3}) if xfip_fip_lhb <= 1.0 else "Under"
    }
