...
st.title("Strikeout Prop Diagnostic (17-Metric + Auto/Manual IP from Outs Line)")
...
bb_rhb = st.number_input("BB% vs RHB", value=0.05)
...
k_lhb = st.number_input("K% vs LHB", value=0.20)
k9_lhb = st.number_input("K/9 vs LHB", value=7.2)
bb_lhb = st.number_input("BB% vs LHB", value=0.10)
...
k_rank = st.number_input("Team K Rank", value=7)
...
pitching_outs_line = st.number_input("Optional: Pitching Outs Line (e.g. 17.5 = 5.2 IP)", value=0.0)
...
if st.button("Analyze"):
    ...
    if pitching_outs_line > 0:
        ...
        ip_source = f"📘 From outs line: {pitching_outs_line} outs = {projected_ip:.2f} IP"
    else:
        projected_ip = 24.0 / avg_bpi
        ip_source = f"🧮 Auto-estimated from avg BPI: {avg_bpi:.2f}"
    ...
    st.subheader("17-Metric Evaluation")
    ...
    if over >= 9:
        ...
        verdict = "⚠️ No clear edge — Don't Touch"
