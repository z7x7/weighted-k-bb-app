...
avg_rhb = st.number_input("AVG vs RHB", value=0.245)
...
avg_lhb = st.number_input("AVG vs LHB", value=0.235)
...
if st.button("Analyze"):
    ...
    projected_ip = pitching_outs_line / 3.0 if pitching_outs_line > 0 else 24.0 / avg_bpi
    ip_source = f"📘 From sportsbook line ({pitching_outs_line} outs)" if pitching_outs_line > 0 else "🧮 Auto-estimated from BPI"
    ...
    results = {
        "AVG vs RHB": evaluate_metric(avg_rhb, {"over": 0.265, "neutral_low": 0.230, "neutral_high": 0.264}),
        "AVG vs LHB": evaluate_metric(avg_lhb, {"over": 0.265, "neutral_low": 0.230, "neutral_high": 0.264}),
    ...
    st.subheader("Results (19-Metric Evaluation including AVG)")
    sorted_results = sorted(results.items(), key=lambda x: ["Over", "Neutral", "Under"].index(x[1]))
    for label, verdict in sorted_results:
        if verdict == "Over":
    ...
    st.write(f"OVER metrics: {over} | NEUTRAL: {neutral} | UNDER: {under}")
